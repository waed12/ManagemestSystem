...
//
	  ...
	  public static Task getInformation(String id) {
		  ...
		  try {
			Connection con=ConnectionDB.getConnection();
			PreparedStatement ps=(PreparedStatement) con.prepareStatement("select task.user_id,user.name, task.text,task.state from task INNER JOIN user where task.user_id=user.user_id and task.user_id='"+id+"'");
			ResultSet result=(ResultSet) ps.executeQuery();
			if(result.next()) {
